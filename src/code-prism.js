...
*
* http://prismjs.com/download.html?themes=prism&languages=markup+css+clike+javascript+bash&plugins=line-numbers 
*/
  ...
  accessors:{
    'codeContent': {
      set: function(code){
        ...
        //wrap content into <pre><code>
        var lang    = 'language-' + this.language || '*';
        var numbers = this['line-numbers'];
        var preElt  = document.createElement('pre');
        var codeElt = document.createElement('code');

        //display line numbers
        if(numbers){
            preElt.className = 'line-numbers';
            if(parseInt(numbers, 10) > 0){
                preElt.setAttribute('data-start', numbers);
            }
        }
       
        codeElt.className = lang; 
        codeElt.innerHTML = code;
        preElt.appendChild(codeElt);

        this.innerHTML = '';    
        this.appendChild(preElt);

        Prism.highlightElement(codeElt, false);
      }
    },
    'language' : {
      attribute : true
